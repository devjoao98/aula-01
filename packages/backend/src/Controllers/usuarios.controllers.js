...
module.exports = {
    ...
    async datails(req,res){
        const user = await Usuario.findOne({ _id: req.params._id })
        res.json(user)
    },

    async delete(req,res){
        const user = await Usuario.findByIdAndDelete({_id: req.params._id})
        res.json(user)
    },
    ...
    async login(req, res){
        ...
        Usuario.findOne({email_usuario:email, tipo_usuario:1}, function(err,user){
            if(err){
                console.log(err)
                res.status(200).json({erro:'Error no servidor. Por favor, tente novamente'})
            }else if (!user){
                res.status(200).json({status:2, error:'E-mail não cadastrado'})
            }else{
                user.isCorrectPassword(senha, async function(err, same){
                    if(err){
                        res.status(200).json({error: 'Error no servidor. Por favor, tente novamente'})
                    }else if(!same){
